...
app.post("/privada", function(req, res) {
    ...
    //creamos llave privada   
        ...
        let comando = `openssl req -x509 -aes-${AesS}-cbc -passout pass:${password} -newkey rsa:${keySize} -keyout key${id}.pem -out cert${id}.pem -sha${shaS} -days ${dias} -nodes -subj "/C=${c}/ST=${st}/L=${l}/O=${o}/OU=${ou}/CN=${cn}" 2>/dev/null && openssl pkcs8 -topk8 -passout "pass:${password}" -in key${id}.pem -out key${id}.key && cat key${id}.key`
        //console.log(comando)
        script = exec(comando);
...
setTimeout(function(){
    let comando = `openssl x509 -CA CA.crt -CAkey private_key.key -in firma${id}.crt -out firmado${id}.crt && cat firmado${id}.crt`
        ...
        script.stdout.on('data', function(data){     
            ...
            res.charset = "UTF-8";
            res.write(data.toString());
            var codificado = btoa(data.toString())
            axios({
